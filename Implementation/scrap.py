            ...
            if os.path.isdir(method): 
                for ooo in os.listdir(method):
                    ...
                    if os.path.isdir(Eq): 
                        ...
                        f.write("echo \"###################################################################################################\" \n")
                        f.write("time timeout 300 java -Djava.library.path=jpf-git/jpf-symbc/lib -Xmx20G -Xms16G -jar target/artifacts/Implementation_jar/Implementation.jar --path1 " + versions[1] + " --path2 " + versions[0] + " --tool A --s coral --b 3 \n")
                        print(Eq)
f.close()
print(index)  
